...
namespace Model3D
{
	struct EdgePoint
	{
		...
		CPoint3D GetNormal(const CRichModel& model) const
		{
			if (isVertex)
				return model.Normal(index);
			auto normal = (1 - proportion) * model.Normal(model.Edge(index).indexOfLeftVert)
				+ proportion * model.Normal(model.Edge(index).indexOfRightVert);
			normal.Normalize();
			return normal;
		}
